...
from utils import to_device, TdnnAffine, for_device_free
...
class SoftmaxLoss(TopVirtualLoss):
    ...
    """
    def init(self, input_dim, num_targets, t=1, reduction='mean', special_init=False, entropy_regularization=0.0):
        self.affine = torch.nn.Linear(input_dim, num_targets)
 
        self.affine.bias.data.fill_(0.0)
        self.affine.weight.data.fill_(0.0)
    ...
    def forward(self, inputs, targets):
        ...
        posterior = self.affine(inputs).unsqueeze(2)
        self.posterior = posterior.detach()

        # The frames-index is 1 now.
        outputs = torch.squeeze(posterior, dim=2)
        result = self.loss_function(outputs, targets)
        if self.entropy_regularization > 0.00000001:
            #breakpoint()
            y_hat_logprob = F.log_softmax(outputs, dim=1)
            result -= self.entropy_regularization * (y_hat_logprob.exp() * y_hat_logprob).sum()
...
class MammLossFunction(nn.Module):
    def __init__(self, n_out, n_classes,  la=20, r=1, easy_margin=False, weight_lambda=0.03, gamma=0.1, tau=0.2, K=5,margin=0.01, scale=15, **kwargs):
        super(MammLossFunction, self).__init__()
        ...
        instance_label = torch.tensor(np.repeat(np.arange(self.C), self.N))
        y_instance_onehot = self.to_one_hot(instance_label, n_dims=self.C)
        class_label = torch.tensor(np.repeat(np.arange(self.C), 1))
        self.centers = torch.nn.Parameter(torch.FloatTensor(n_out, self.C*self.N), requires_grad=True)
        ...
        self.register_buffer('instance_label', instance_label)
        self.register_buffer('y_instance_onehot', y_instance_onehot)
        self.register_buffer('class_label', class_label)
        self.posterior = None
        return
    ...
    def forward(self, input, target):
        ...
        positive_mask=torch.eq(target.view(-1,1)-self.instance_label.view(1,-1),0.0).float() #obtain positive mask
        ...
        y_target_onehot = self.to_one_hot(target, n_dims=self.C).to(logits.device)
        logits_mask=1-torch.eq(logits,0.0).float().to(logits.device)
        predict=self.scale_mask_softmax(logits, logits_mask,1).to(logits.device)
        ...
        output_tem = output.detach()
        label_tem = target.detach()
        loss = self.ce(output, target)
        self.posterior = output_tem.unsqueeze(2)
        #breakpoint()
        if self.weight_lambda  > 0:
            sim_center = centers.t().matmul(centers)
            centers_logits = torch.matmul(sim_center , self.y_instance_onehot)
            y_target_onehot = self.to_one_hot(self.instance_label, n_dims=self.C)
            logits_mask=1-torch.eq(centers_logits,0.0).float()
            predict=self.scale_mask_softmax(centers_logits, logits_mask,1)
            cosine = predict
            sine = torch.sqrt((1.0 - torch.pow(cosine, 2)).clamp(0, 1))
            phi = cosine * self.cos_m - sine * self.sin_m
            if self.easy_margin:
            ...
            return loss+self.weight_lambda*reg
        else:
            return loss
